...
export async function GET(
  ...
  try {
    ...
    const scale = await prisma.scale.findUnique({
      where: { id },
      select: { id: true, lastWeight: true, lastWeightAt: true, isActive: true },
    })
    ...
    return NextResponse.json({ error: "Failed to fetch scale weight" }, { status: 500 })
  }
}
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to update scale weight" }, { status: 500 })
  }
}
